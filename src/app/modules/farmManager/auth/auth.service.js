...
const loginFarmManager = async (email, password) => {
  ...
  // 2. Check if user has FARM_ADMIN or MANAGER role
  if (user.role !== Role.FARM_ADMIN && user.role !== Role.MANAGER) {
...
export const FarmManagerAuthService = {
  loginFarmManager,
  ...
  async forgotPassword(email) {
    ...
    // 2. Verify user is FARM_ADMIN or MANAGER
    if (user.role !== Role.FARM_ADMIN && user.role !== Role.MANAGER) {
      throw new DevBuildError(
        "This endpoint is only for Farm Managers",
